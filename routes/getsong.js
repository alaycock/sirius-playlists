...
const SIRIUS_PREFIX =
  "http://www.siriusxm.com/metadata/pdt/en-us/json/channels/";
...
function getRadioData(time, songData, channel, callback) {
  ...
  request({ uri: sirius_source }, function(err, response, body) {
    ...
    if (
      ...
      !data.channelMetadataResponse.messages ||
      data.channelMetadataResponse.messages.code != 100
    ) {
      console.log(data);
      callback("Could not get song from SiriusXM.");
      return;
      ...
      songData.artist =
        data.channelMetadataResponse.metaData.currentEvent.artists.name;
      songData.title =
        data.channelMetadataResponse.metaData.currentEvent.song.name;
    }
